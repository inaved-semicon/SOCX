...
module TOP #(
		...
		parameter ALU_OPER_WIDTH    = DATA_WIDTH,
		parameter ALU_OUT_WIDTH     = ALU_OPER_WIDTH*2,
		parameter CLKDIV_WIDTH      = DATA_WIDTH
)
(
 input   wire                          RST_N,
 input   wire                          UART_CLK,
...
RST_SYNC #(.NUM_STAGES(SYNC_STAGES))U0_RST_SYNC_REF(
	.CLK(REF_CLK),
	.RST(RST_N),
	.SYNC_RST(SYNC_RST_REF)
	);

RST_SYNC #(.NUM_STAGES(SYNC_STAGES))U1_RST_SYNC_UART(
	.CLK(UART_CLK),
	.RST(RST_N),
...
///////////////////// Data Synchronizers /////////////////////
///********************************************************///
DATA_SYNC #(.NUM_STAGES(SYNC_STAGES),.BUS_WIDTH(DATA_WIDTH))U0_DATA_SYNC(
...
Async_fifo #(.D_SIZE(DATA_WIDTH),.P_SIZE(FIFO_PTR_WIDTH),.F_DEPTH(FIFO_DEPTH))U0_Async_fifo(
.i_w_clk(REF_CLK),
...
CLKDIV_MUX #(.WIDTH(CLKDIV_WIDTH))U0_CLKDIV_MUX(
.IN(UART_Config[7:2]),
.OUT(Div_Ratio_to_RX)
);
...
SYS_CTRL #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH)) U0_SYS_CTRL (
	.CLK(REF_CLK),
	.RST(SYNC_RST_REF),
...
/////////////////////// Register File ////////////////////////
///********************************************************///
Register_File #(.ADDR_WIDTH(ADDR_WIDTH),.DATA_WIDTH(DATA_WIDTH),.MEM_DEPTH(RF_DEPTH))U0_Register_File(
	.WrData(Wr_Reg),
	.Adresss(Addr),
	.WrEn(WrEn),
	.RdEn(RdEn),
...
//////////////////////////// ALU /////////////////////////////
///********************************************************///
ALU_16B #(.OPER_WIDTH(ALU_OPER_WIDTH),.OUT_WIDTH(ALU_OUT_WIDTH))U0_ALU_16B(
	.A(Operand_A),
	.B(Operand_B),
	.EN(ALU_EN),
	.ALU_FUN(ALU_FUN),
